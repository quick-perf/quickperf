...
public class MaxQueriesSendingVerifier implements VerifiablePerformanceIssue<ExpectMaxQueriesSending, SqlAnalysis>  {

    public static final MaxQueriesSendingVerifier INSTANCE = new MaxQueriesSendingVerifier();

    private MaxQueriesSendingVerifier() { }

    @Override
    public PerfIssue verifyPerfIssue(ExpectMaxQueriesSending annotation, SqlAnalysis sqlAnalysis) {
    ...
    private String buildBaseDescription(Count sendingNumber, Count expectedSendingNumber) {
        ...
        return    "You may think that there " + (severalExpectedSendings ? "were" : "was")
                + " at most <" + expectedSendingNumber.getValue() + ">"
