...
public interface ExtractablePerformanceMeasure<R extends PerfRecord, M extends PerfMeasure> {
    ...
    ExtractablePerformanceMeasure RECORD_IS_PERF_MEASURE = new ExtractablePerformanceMeasure() {
        @Override
        public PerfMeasure extractPerfMeasureFrom(PerfRecord perfRecord) {
           if(!(perfRecord instanceof PerfMeasure)) {
               return PerfMeasure.NONE;
           }
            return (PerfMeasure) perfRecord;
        }
    };

    M extractPerfMeasureFrom(R perfRecord);
